...
class Ucontext4c_Base
{
	public static $name			= 'ucontext4c';
	...
	public static function initBase()
	{
		if (!self::$table)
		{
			...
			self::$table = array(
			'cache'			=> $wpdb->base_prefix.'ucontext4c_cache',
			'click_log'		=> $wpdb->base_prefix.'ucontext4c_click_log',
			'keyword'		=> $wpdb->base_prefix.'ucontext4c_keyword',
			'spider_agent'	=> $wpdb->base_prefix.'ucontext4c_spider_agent'
			);
		}
	}
	...
	{
		if ($force || !(int)get_post_meta($post_id, 'ucontext4c_last_process', 0))
		{
			if ((int)$post_id && !wp_is_post_revision($post_id))
			{
				...
				$keyword_hints = get_post_meta($post_id, 'ucontext4c_manual_keywords', true);

				$auto_keywords = Ucontext4c_Keyword::findKeywordsInContent($post->post_title, $post->post_content, $keyword_hints);

				update_post_meta($post_id, 'ucontext4c_auto_keywords', serialize($auto_keywords));
				update_post_meta($post_id, 'ucontext4c_last_process', current_time('timestamp'));
	...
	public static function saveKeywordsToMainList($keyword_list, $type = 'auto')
	{
		...
		if ($type == 'auto' && (int)@get_option('ucontext4c_no_autokeywords', 0))
		{
			return NULL;
		}
	...
	{
		$last_check = (int)get_option('ucontext4c_last_version_check', 0);

		if ($last_check < (current_time('timestamp') - 86400))
		{
			$response = wp_remote_post('http://www.ucontext.com/api.php?method=getPluginVersion&integration='.urlencode(UCONTEXT4C_INTEGRATION_HANDLE));

			if (!is_wp_error($response))
			{
				$version = preg_replace('/[^0-9\.]+/is', '', $response['body']);

				update_option('ucontext4c_latest_version', $version);
			}
		}

		update_option('ucontext4c_last_version_check', current_time('timestamp'));
	}
