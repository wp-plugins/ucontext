...
class Ucontext4c_Form
{
	...
	function textField($label, $name, $value, $size = 50, $caption = '', $required = FALSE)
	{
		...
		echo '<input type="text" name="'.$name.'" value="'.htmlspecialchars($value).'" size="'.$size.'" onchange="ucontext4c_form_changed = true;" />';
		if ($required)
		{
	...
	function checkboxField($label, $name, $value, $caption = '', $required = FALSE)
	{
		...
		echo '<input type="checkbox" name="'.$name.'" value="1"'.$checked.' onchange="ucontext4c_form_changed = true;" />';
		if ($required)
		{
			echo '<span class="required">Required</span>';
		}
	...
	function textareaField($label, $name, $value, $rows = 3, $cols = 40, $caption = '', $max_length = NULL, $required = FALSE)
	{
		...
		echo '<tr><th>'.$label.':</th><td>';
		echo '<textarea name="'.$name.'" rows="'.$rows.'" cols="'.$cols.'"'.$max_length.' onchange="ucontext4c_form_changed = true;">'.htmlspecialchars($value).'</textarea>';
		if ($required)
		{
	...
	{
		echo '<tr><th>'.$label.':</th><td>';
		echo '<select name="'.$name.'" onchange="ucontext4c_form_changed = true;">';
	...
	function startTable()
	{
		echo '<table class="ucontext4c_form_table" cellpadding="0" cellspacing="5">';
	}

	function fadeSave()
	{
		echo '<div id="ucontext4c_saved" style="display: none; padding: 10px;">Saved</div>';
		if (isset($_GET['saved']) && $_GET['saved'])
		{
			echo '<script language="Javascript">ucontext4c_fadeSaved("ucontext4c_saved");</script>';
		}
	}
	...
	function listErrors($error_list)
	{
		if (is_array($error_list) && count($error_list))
		{
			...
			echo '<ul class="ucontext4c_error_list">';
